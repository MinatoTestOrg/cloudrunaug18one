...
export class GridComponent implements OnInit {
  ...
  findPage() {
    ...
    if (currentUserData) {
      if (this.acRoute.snapshot?.queryParams?.id && (this.acRoute.snapshot?.queryParams?.id == this.gridConfig?.parentId) && this.gridConfig.ajaxUrl?.url?.includes('/datatable') && this.gridConfig.ajaxUrl?.url?.includes('/rest/')) {
        this.currentPage = this.currentPage + "_" + this.gridConfig?.ajaxUrl?.url?.split('/')[2] + "_" + currentUserData?.sid;
      } else {
        this.currentPage = this.currentPage + "_" + currentUserData?.sid;
      }
    }
  }
